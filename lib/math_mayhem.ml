(* open! Core open! Async
   ...
   probably be in a loop *) let operator = if Random.int 2 % 2 = 0 then " + "
   else " - " in let first = Random.int 41 - 20 in (* random number from -20
   to 20 *) let second = Random.int 41 - 20 in let correct_answer = first +
   if String.equal operator " + " then second else second * -1 (* if the
   operand is subtract, then we are subtracting *) in let question_string =
   (* the question we will show to the client *) Int.to_string first ^
   operator ^ Int.to_string second in (* this will probably be the end of the
   loop *)
