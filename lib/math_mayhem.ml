...
participating in the minigame *) 
let initialize (players :(Socket.Address.Inet.t * Player.t) list) = 
  { participants = players } 
;;

(* 1. i need timer to stop the game... *) 
(*use an ivar with and check after X seconds -> when it's full, stop, change the game (maybe make a winner variant, whatever )
*)
...
let first = Random.int 41 - 20 in (* random number from -20 to 20 *) 
let second = Random.int 41 - 20 in 
let correct_answer = first + if String.equal operator " + " then second else second * -1 (* if the operand is subtract, then we are subtracting *) in
let question_string = (* the question we will show to the client *)
Int.to_string first ^ operator ^ Int.to_string second in 
(* this will
probably be the end of the loop *)
...
a $$ deduction instead *) (* 5. keep track of score - lowest score dies *)
